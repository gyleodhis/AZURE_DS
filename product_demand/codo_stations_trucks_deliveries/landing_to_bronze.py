...
# load landing paths 
codo_truck_deliveries_loaded_df_arr = []
dates = generate_date(start_date , end_date,"%Y%m%d")
for day in dates:
    landing_path = f"{container_var}landing/product_demand/codo_stations_truck_deliveries/{day}/codo_stations_truck_deliveries.csv"
    try:
        ...
        codo_truck_deliveries_loaded_df_arr.append(each_day_df)
    except:
...
# COMMAND ----------
 ...
 if (len(codo_truck_deliveries_loaded_df_arr) > 0):   
    codo_truck_deliveries_merged_df = codo_truck_deliveries_loaded_df_arr[0]
    for x in range(len(codo_truck_deliveries_loaded_df_arr)):
        if(x > 0):
           codo_truck_deliveries_merged_df = codo_truck_deliveries_merged_df.unionAll(codo_truck_deliveries_loaded_df_arr[x])
else :
...
final_codo_stations_truck_deliveries_df = codo_truck_deliveries_merged_df.select([col(cols).alias(cols.replace(' ','_').upper()) for cols in codo_truck_deliveries_merged_df.columns])

# COMMAND ----------

#Add water mark columns 
final_codo_stations_truck_deliveries_df = (final_codo_stations_truck_deliveries_df                                      
                      .withColumn("DELIVERY_DATE",date_format("DELIVERY_DATE","yyyyMMdd"))                      
...
unique_dates = final_codo_stations_truck_deliveries_df.select("DELIVERY_DATE").distinct().rdd.flatMap(lambda x: x).collect()
...
print(final_codo_stations_truck_deliveries_df.count())

# COMMAND ----------

"""WRITTING IN STATION TRUCK DELIVERIES"""
(final_codo_stations_truck_deliveries_df.write.format("delta")
 .partitionBy("DELIVERY_DATE")
 .option("replaceWhere",  f"{partition_predicate}")
.mode("overwrite").save(bronze_path))

final_codo_stations_truck_deliveries_df.unpersist(blocking=True)
