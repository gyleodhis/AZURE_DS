...
# load landing paths 
codo_stock_volume_loaded_df_arr = []
dates = generate_date(start_date , end_date,"%Y%m%d")
for day in dates:
    landing_path = f"{container_var}landing/product_demand/codo_stations_stock_volume/{day}/codo_stations_stock_volume.csv"
    try:
        ...
        codo_stock_volume_loaded_df_arr.append(each_day_df)
    except:
...
if (len(codo_stock_volume_loaded_df_arr) > 0) :  
    stock_vol_loading_merged_df = codo_stock_volume_loaded_df_arr[0]    
    for x in range(len(codo_stock_volume_loaded_df_arr)):
        if(x > 0):
            stock_vol_loading_merged_df = stock_vol_loading_merged_df.unionAll(codo_stock_volume_loaded_df_arr[x])
else :
    dbutils.notebook.exit("No files to load")

# COMMAND ----------

final_codo_station_stock_volume_df = stock_vol_loading_merged_df.select([col(cols).alias(cols.replace(' ','_').upper()) for cols in stock_vol_loading_merged_df.columns])

# COMMAND ----------

#Add water mark columns 
final_codo_station_stock_volume_df = (final_codo_station_stock_volume_df                                      
...
unique_dates = final_codo_station_stock_volume_df.select("SALES_DATE").distinct().rdd.flatMap(lambda x: x).collect()
unique_record_ids = final_codo_station_stock_volume_df.select("RECORD_ID").distinct().rdd.flatMap(lambda x: x).collect()
...
for rec_ids in unique_record_ids : 
    record_ids_processed.append(str(rec_ids))
...
record_ids_processed_str = ",".join(str(i) for i in record_ids_processed)
...
print(final_codo_station_stock_volume_df.count())

# COMMAND ----------

"""WRITTING IN STATION STOCK VOLUME"""
(final_codo_station_stock_volume_df.write.format("delta")
 .partitionBy("SALES_DATE")
 .option("replaceWhere",  f"{partition_predicate}")
.mode("overwrite").save(bronze_path))

final_codo_station_stock_volume_df.unpersist(blocking=True)
