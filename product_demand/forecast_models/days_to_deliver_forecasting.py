...
model_path = f"{container_var}gold/product_demand/codo_next_delivery_model/"

# COMMAND ----------

"""READING CODO FEATURE STORE """                       
codo_df = (spark.read.format('delta')
    .load(gold_path).where(f"RRUN_DATE_ID = {run_date_int} AND isNotNull(SALES_DATE)")
    )
...
codo_raw_df = codo_raw_df.withColumnRenamed('SALES_DATE','ds').withColumnRenamed('NEXT_DELIVERY_DAYS','y')
...
merged_forecasted_sales_df = merged_forecasted_sales_df.withColumnRenamed('ds','FORECASTED_SALES_DATE').withColumnRenamed('yhat','FORCASTED_NEXT_DELIVERY_DAYS').withColumnRenamed('y','ACTUAL_VALUE').withColumn("FORECASTED_SALES_DATE",date_format("FORECASTED_SALES_DATE","yyyy-MM-dd"))
