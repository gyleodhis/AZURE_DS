...
silver_path = f"{container_var}silver/product_demand/codo_station_meter_reading/"


# COMMAND ----------

"""READING IN STATION METER READING """                       
codo_meter_reading_df = (spark.read.format('delta')
    .load(bronze_path).where(f"RUN_DATE_ID = {run_date_int}"))

# COMMAND ----------

codo_meter_reading_df = (codo_meter_reading_df
...
codo_meter_reading_df = codo_meter_reading_df.select("RECORD_ID",
                                    "STATION",
                                    "METER_DESCRIPTION",
                                    "PRODUCT",
                                    col("PREVIOUS_READING").cast("int").alias("PREVIOUS_READING"),
                                    col("CURRENT_READING").cast("int").alias("CURRENT_READING"),
                                    "VOLUME",                                  
                                    "TYPE",                                    
                                    "UNIT_PRICE",
                                    "SALES_VALUE",
                                    ...
                                    "STATUS",
                                    col("SALES_DATE").cast("int").alias("SALES_DATE"),
                                    "DATE_CREATED",
                                    "FILE_NAME"
                                    )
...
clean_codo_meter_reading_df = (codo_meter_reading_df                      
...
unique_dates = clean_codo_meter_reading_df.select("SALES_DATE").distinct().rdd.flatMap(lambda x: x).collect()
...
"""WRITTING IN STATION METER READING """
(clean_codo_meter_reading_df.write.format("delta")
